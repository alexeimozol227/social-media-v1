...
* **PR #2 (этот PR)** — Фаза 0 + Фаза 1 Спринт 1 «skeleton»: монорепо, лендинг, самописная auth (register / login / refresh / logout / me) с JWT (15 мин) + refresh-семьями в HttpOnly cookie + replay-детекцией, фундаментальный tenancy-слой (workspaces / workspace_members / brands), RLS-контекст через `SET LOCAL`, typed `AppError`, pytest auth + RLS, GitHub Actions CI.

Email verification, MFA (TOTP), password reset, skill-инфраструктура, event bus, WebSocket, Unleash, idempotency, OpenTelemetry, retention pg_cron, audit_events — следующие PR этого же Спринта 1.
